...
class _GeneratePaymentTabState extends State<_GeneratePaymentTab> {
  ...
  /// Builds 10 past + current + 10 future Thursday-ending weeks.
  /// Auto-selects the week that contains today.
  void _buildWeeks() {
    _weeks = _thursdayWeeks();
    // Find and select the week that contains today
  ...
  static List<_ThursdayWeek> _thursdayWeeks() {
    ...
    // Find the Thursday of the current week (Friday→next Thu, or today if Thu)
    DateTime currentThu = today;
    while (currentThu.weekday != DateTime.thursday) {
      currentThu = currentThu.add(const Duration(days: 1));
    }
    // If today is Thursday, that IS the end of current week
    // If today is past Thursday but not yet next Thu, currentThu = next Thu
    // We want the Thursday whose week CONTAINS today:
    // week = (Thu-6) to Thu
    // So if today is after (currentThu-6) and <= currentThu, currentThu is correct
    // else step back one week
    final currentWeekStart = currentThu.subtract(const Duration(days: 6));
    if (today.isBefore(currentWeekStart)) {
      currentThu = currentThu.subtract(const Duration(days: 7));
    }

    // Build: 10 weeks before current + current + 10 weeks after = 21 total
    // Index 10 = current week
    for (int offset = -10; offset <= 10; offset++) {
      final weekEnd   = currentThu.add(Duration(days: 7 * offset));
      final weekStart = weekEnd.subtract(const Duration(days: 6));
      final label     = '${fmt.format(weekStart)} – ${fmt.format(weekEnd)}';
      weeks.add(_ThursdayWeek(weekStart, weekEnd, label));
    }
    // weeks[0] = 10 weeks ago, weeks[10] = current, weeks[20] = 10 weeks ahead
    return weeks;
  }
