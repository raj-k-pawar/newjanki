...
class _AllCustomersScreenState extends State<AllCustomersScreen> {
  ...
  Future<void> _pickDate(BuildContext context) async {
    final picked = await showDatePickerDialog(context, initial: _filterDate);
    if (picked != null) { _filterDate = picked; _apply(); }
  }
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (c.checkInDate != null) ...[
                Text('Check-In:  \${DateFormat(\'dd MMM yyyy\').format(c.checkInDate!)}',
                    style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w500, color: AppColors.primary)),
                Text('Check-Out: \${c.checkOutDate != null ? DateFormat(\'dd MMM yyyy\').format(c.checkOutDate!) : \'—\'}',
                    style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textMedium)),
                Text('\${c.stayNights} Night(s)',
                    style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textMedium)),
              ] else
              Text('Visit: ${DateFormat('dd MMM yyyy').format(c.visitDate)}',
                  style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textLight)),
              const SizedBox(height: 8),
              // Food options
