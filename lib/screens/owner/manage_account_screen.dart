...
class _GenerateReportTabState extends State<_GenerateReportTab> {
  ...
  Future<void> _generate() async {
    ...
    // Priority 1: Online customers
    for (final c in online) {
      if (limit > 0 && running >= limit) break;
      final include = limit <= 0 ? c.totalAmount
          : (running + c.totalAmount <= limit ? c.totalAmount : limit - running);
      if (include <= 0) break;
      running += include;
    ...
    // Priority 2: Cash customers (only if taxable limit not yet reached)
    for (final c in cash) {
      if (limit > 0 && running >= limit) break;
      final include = limit <= 0 ? c.totalAmount
          : (running + c.totalAmount <= limit ? c.totalAmount : limit - running);
